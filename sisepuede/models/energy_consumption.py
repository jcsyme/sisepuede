...
class EnergyConsumption:
    ...
    def get_agrc_lvst_prod_and_intensity(self,
        ...
        vec_inen_energy_intensity_agrc_lvst = np.nan_to_num(vec_inen_energy_intensity_agrc_lvst/vec_inen_prod_agrc_lvst, nan = 0.0, posinf = 0.0)
    ...
    def get_fgtv_array_for_fugitive_emissions(self,
        ...
        arr_ef_per_config_energy = np.nan_to_num(arr_ef_per_config_energy, nan = 0, posinf = 0)
    ...
    def project_energy_consumption_by_fuel_from_effvars(self,
        ...
        for modvar_fuel_frac in dict_fuel_fracs.keys():
            ...
            arr_frac_norm += np.nan_to_num(arr_frac/arr_efficiency, nan = 0.0)

        # project demand forward
        arr_demand = np.nan_to_num(arr_consumption/arr_frac_norm, nan = 0.0)
        ...
        for modvar_fuel_frac in dict_fuel_fracs.keys():
            ...
            arr_consumption_fuel = np.nan_to_num(arr_demand*arr_frac/arr_efficiency, nan = 0.0)
    ...
    def project_transportation(self,
        ...
        for category in dict_trde_cats_to_trns_vars.keys():
            ...
            if category == self.cat_trde_frgt:
                array_trde_vehicle_dem_cur_by_cat = np.nan_to_num(array_trde_dem_cur_by_cat/array_trns_avg_load_freight, nan = 0.0, neginf = 0.0, posinf = 0.0)*scalar_tnrs_length_demfrieght_to_dempass
                array_trns_total_mass_distance_demand += array_trde_dem_cur_by_cat
            else:
                array_trde_vehicle_dem_cur_by_cat = np.nan_to_num(array_trde_dem_cur_by_cat/array_trns_occ_rate_passenger, nan = 0.0, neginf = 0.0, posinf = 0.0)
