...
class AFOLU:
    ...
    def _initialize_array_classes(self,
        ...
        self.arrays_soil = coll_arrays.ArraysSOIL(
            df_afolu_trajectories, 
            self.model_attributes, 
        )
    ...
    def build_c_conversion_matrices(self,
        ...
        # get variables
        arr_bmass_ag = self.arrays_lndu.arr_lndu_biomass_stock_factor_ag
        arr_bmass_bgr = self.arrays_lndu.arr_lndu_biomass_stock_ratio_bg_to_ag
    ...
    def build_varsout_agrc_demand_balance_from_ilu(self,
    ...
    def build_varsout_lndu_from_ilu(self,
        ...
        # some variable conversion
        modvar_ilu_area, _ = self.get_modvars_for_unit_targets_ilu()
        scalar_area_ilu_to_config_area = self.model_attributes.get_scalar(
            modvar_ilu_area, 
            "area"
        )


        ##  SOME OTHER DFS THAT ARE RETURNED
        ...
        # note that no scalar is applied
        df_land_use_ilu = self.model_attributes.array_to_df(
            arr_land_use, 
        ...
        arrs_lndu_conv_to = self.get_lndu_area_converted_into_from_convmats(
            arrs_lndu_land_conv, 
        )
        arrs_lndu_conv_from = self.get_lndu_area_converted_away_from_convmats(
            arrs_lndu_land_conv, 
        )
        ...
        df_lndu_area_conv_matrices = self.format_transition_matrix_as_input_dataframe(
            arrs_lndu_land_conv*scalar_area_ilu_to_config_area,
        ...
        df_out = [
            ...
            self.model_attributes.array_to_df(
                arr_land_use*scalar_area_ilu_to_config_area, 
                self.modvar_lndu_area_by_cat
            ),
            self.model_attributes.array_to_df(
                arrs_lndu_conv_from*scalar_area_ilu_to_config_area, 
                self.modvar_lndu_area_converted_from_type
            ),
            self.model_attributes.array_to_df(
                arrs_lndu_conv_to*scalar_area_ilu_to_config_area, 
    ...
    def build_varsout_lvst_demand_balance_from_ilu(self,
    ...
    def convert_fuelwood_to_biomass_c_equivalent(self,
        ...
        # get mass of C; biomass*C frac
        vec_mass_c = (
            vec_mass_biomass
            *self.arrays_frst.arr_frst_frac_c_per_dm    # is a vector
        )
    ...
    def extract_variables_from_ledger(self,
        ...
        # 8. get CO2 conversion emissions in above-ground biomass
        ...
        # 9. get CO2 conversion emissions in above-ground biomass
    ...
    def get_agrc_variables_additional(self,
        ...
        """Get emissions in AGRC from:
            ...
            * Other variables:
                - Quantity of residues by pathway and crop type
        """
        ...
        # initialize with emissins from co2 residues (used for tracking)
        ...
        # add in 
        df_out += self.get_agrc_residue_output_pathways()

        return df_out
    


    def get_agrc_emissions_co2_residues_posneg(self,
    ) -> List[pd.DataFrame]:
        ...
        vec_frac_dm = self.arrays_frst.arr_frst_frac_c_per_dm
    ...
    def get_agrc_emissions_residue_burning(self,
    ) -> List[pd.DataFrame]:
        """Retrieve CH4 and N2O emissions from residue burning. 

        NOTE: relies on arrays stored in self.arrays_agrc; cannot be run without
            first populating:
        ...
        Equations
        ---------
        BIOMASS BURNING (FROM EQUATION 2.27 & V4 SECTION 5.3.4.1)
        """
        ...
        # total residues burned; CHECK
        #   - total sent to "burn" pathway
        #   - combustion factor
        #   - below-ground residues lost
        vec_agrc_crop_residue_burned = np.sum(
            self.arrays_agrc.arr_agrc_residue_final_use_burned
            * (1 + self.arrays_agrc.arr_agrc_ratio_below_ground_biomass_to_above_ground_biomass)
            * self.arrays_agrc.arr_agrc_combustion_factor, 
            axis = 1,
        )
        ...
        vec_agrc_emissions_ch4_residue_and_biomass_burning = (
            ...
            * self.arrays_agrc.arr_agrc_ef_ch4_burning
        ...
        vec_agrc_emissions_n2o_residue_and_biomass_burning = (
            vec_agrc_crop_residue_burned
            * self.arrays_agrc.arr_agrc_ef_n2o_burning
            * self.model_attributes.get_scalar(
                modvar_ilu_mass,
                "mass",
            )
    ...
    def get_agrc_residue_output_pathways(self,
    ) -> List[pd.DataFrame]:
        """Get emissions associated with total residues. Excludes emissions 
            associated with residue use in energy, since that is handled in 
            Energy.
        ...
        ##  BUILD ARRAYS FOR EACH OF THE RESIDUE PATHWAYS

        # burned
        arr_final_use_burned = (
            self.arrays_agrc.arr_agrc_residue_final_use_burned
            * self.model_attributes.get_variable_unit_conversion_factor(
                modvar_ilu_mass,
    ...
    def get_bcl_growth_rates_and_stocks(self,
        ...
        **kwargs :
        ...
        arr_stock_init = self.scale_bcl_mass_per_area_array(
            self.arrays_lndu.arr_lndu_biomass_stock_factor_ag,
            self.modvar_lndu_biomass_stock_factor_ag,
    ...
    def get_lndu_area_converted_away_from_convmats(self,
        arrs_lndu_land_conv: np.ndarray,
    ) -> np.ndarray:
        """Shortcut function to calculate the area converted to each type from
            conversion matrices.
        """
        arrs_lndu_conv_away = np.array(
            [
                np.sum(x - np.diag(np.diagonal(x)), axis = 1) 
                for x in arrs_lndu_land_conv
            ]
        )

        return arrs_lndu_conv_away



    def get_lndu_area_converted_into_from_convmats(self,
        arrs_lndu_land_conv: np.ndarray,
    ) -> np.ndarray:
        """Shortcut function to calculate the area converted to each type from
            conversion matrices.
        ...
        arrs_lndu_conv_into = np.array(
            [
                np.sum(x - np.diag(np.diagonal(x)), axis = 0, )
                for x in arrs_lndu_land_conv
            ]
        )

        return arrs_lndu_conv_into
    ...
    def get_lndu_area_drained_organic_soils(self,
        ...
        """Get the area of drained organic soils in terms of arr_land_use
            areas.
    ...
    def get_frst_inventory_aligned_sequestration(self,
        arrs_lndu_land_conv: np.ndarray,
    ) -> pd.DataFrame:
        """National inventories account for sequestration as a carbon potential
            times area converted. This calcualtion allows for closer calculation
            of sequestration to inventories.

        Function Arguments
        ------------------
        arrs_lndu_land_conv : np.ndarray
            In terms of ILU area
        """

        ##  INITIALIZATION

        # get model variable for area basis
        modvar_ilu_area, _ = self.get_modvars_for_unit_targets_ilu()
        modvar_out = self.modvar_frst_emissions_co2_sequestration_potential

        # get ordered LNDU indices for forests (same as col ordering of output var)
        cats_frst = self.model_attributes.get_variable_categories(modvar_out, )
        inds_lndu_ord = self.get_lndu_indices_fstp_fsts(cats_ordered = cats_frst, )


        ##  GET ARRAYS AND CONVERSIONS

        # get area converted to each land use class
        arrs_lndu_conv_to = self.get_lndu_area_converted_into_from_convmats(
            arrs_lndu_land_conv,
        )

        # biomass factors in terms of all forest classes
        arr_lndu_stock = (
            self.scale_bcl_mass_per_area_array(
                self.arrays_lndu.arr_lndu_biomass_stock_factor_ag,
                self.modvar_lndu_biomass_stock_factor_ag,
                modvar_ilu_area,
                None,
            )
            *(1 + self.arrays_lndu.arr_lndu_biomass_stock_ratio_bg_to_ag)
        )

        scalar_to_config = self.model_attributes.get_scalar(
            self.modvar_lndu_biomass_stock_factor_ag,
            "mass",
        )

        # get sequestration potential--the area of land converted to forest*secondary stock
        arr_frst_seq_potential = -1*(
            sf.do_array_mult(
                (arr_lndu_stock*arrs_lndu_conv_to)[:, inds_lndu_ord],
                self.arrays_frst.arr_frst_frac_c_per_dm,
            )
            *self.factor_c_to_co2
            *scalar_to_config
        ) 
        # set output, should be in terms of 
        df_out = self.model_attributes.array_to_df(
            arr_frst_seq_potential,
            modvar_out,
        )
    ...
    def get_lndu_indices_fstp_fsts(self,
        cats_ordered: Union[List[str], None] = None,
        include_mangroves: bool = False,
        return_categories: bool = False,
    ) -> Tuple:
        """Get the indices of primary forest and secondary forest in
            Land Use. Ordered for specific use in BCL contexts.
        
        Returns
        -------
        For return_categories == False (default)
        - If cats_ordered is None (default):
            * if include_mangroves == False:
                (ind_lndu_fstp, ind_lndu_fsts, )
            * if include_mangroves == True:
                (ind_lndu_fstm, ind_lndu_fstp, ind_lndu_fsts)
            Otherwise, returns lndu indices in order of cats_ordered

         For return_categories == True
            Returns lndu categories only (not lndu indices) in ordering
            described above

        Keyword Arguments
        -----------------
        cats_ordered
            Option to specify ordered forest categories using cats_ordered;
            overrides ordering suited to BCL (don't use in that context)
        include_mangroves : bool
            Include mangroves in the ordering?
        return_categories : bool 
            Return categories only?
        """

        # initialize output
        if not sf.islistlike(cats_ordered):
            cats_ordered = [self.cat_frst_prim, self.cat_frst_scnd]
            if include_mangroves: 
                cats_ordered = [self.cat_frst_mang] + cats_ordered
        
        cats_ordered = [x for x in cats_ordered if x in self.attr_frst.key_values]
        
        # get the output categories (in terms of land use)
        out = [self.dict_cats_frst_to_cats_lndu.get(x) for x in cats_ordered]
        if return_categories:
            return tuple(out)
        
        # otherwise, get ordered indices
        out = tuple([self.attr_lndu.get_key_value_index(x, ) for x in out])
    ...
    def get_lndu_n_volatilisation_factor_ef4(self,
        ...
        for modvar_lndu_frac_drywet, arr_lndu_fdw in dict_arrs_lndu_frac_drywet.items():
            ...
            _, ind_soil = self.get_soil_cat_ind_from_modvar(modvar_lndu_frac_drywet, )
        ...
        # set output tuple
        out = arr_lndu_ef4_n_volatilisation
    ...
    def get_lndu_variables_other(self,
        ...
        """Get:
            ...
            * Emissions
                ...
                - Inventory aligned-sequestration (seq. potential by forest
                    type)
            * Other
        ...
        df_out = [
            ...
            # inventory aligned sequestration potential
            self.get_frst_inventory_aligned_sequestration(
                arrs_lndu_land_conv,
            ),
    ...
    def get_lndu_soil_soc_factors(self,
        ...
        ------------------
        arr_agrc_crop_area : np.ndarray
            Array giving crop areas in terms of ILU
        arr_lndu_prevalence : np.ndarray
            Array giving land use prevalence in terms of ILU

        Keyword Arguments
        -----------------
        default_fmg_grassland_not_pasture : float
            F_MG for grasslands not pasture. Default is 1 (can be read in later)
        """

        ##  SET SOME VARIABLES

        # fraction of croplands using no-till
        arr_agrc_frac_improved_by_crop = self.arrays_agrc.arr_agrc_frac_no_till
        ...
        vec_lndu_frac_pastures_improved = self.arrays_lndu.arr_lndu_frac_pastures_improved
        ...
        vec_lndu_frac_cropland_improved = (
            arr_agrc_crop_area_improved.sum(axis = 1, )
            /arr_lndu_prevalence[:, self.ind_lndu_crop]   # should be a denominator of 1, but just to be safe
        )
        ...
        vec_agrc_frac_residue_inputs = (
            np.nan_to_num(
                sf.do_array_mult(
                    arr_agrc_crop_area*arr_agrc_frac_residue_inputs,
                    1/arr_agrc_crop_area.sum(axis = 1),
                ),
                nan = 0.0,
                posinf = 0.0,
            )
        ...
        # get the inputs component from residues
        vec_agrc_inputs_component = (
            sf.vec_bounds(
    ...
    def get_soil_arrs_ef_c_drained_organic_soils(self,
        ...
        """Get arrays for C drained organic soils emission factors. Converts to
            ...
            (a) output mass units
            (b) area of modvar_ilu_area 
                (modvar_ilu_area from self.get_modvars_for_unit_targets_ilu())
    ...
    def get_soil_cat_ind_from_modvar(self,
        modvar: 'ModelVariable',
    ) -> Tuple[str, int]:
        """Commonly called throughout the SOIL subsector. Retrieves a soil 
            category and associated index in SOIL subsector from the modvar.
        """
        # try retrieving
        cat_soil = self.model_attributes.get_variable_attribute(
            modvar, 
            self.pycat_soil,
        )
        ...
        cat_soil = clean_schema(cat_soil, )
    ...
    def get_soil_ef1_components_fcr(self,
        arr_agrc_frac_n_content_ag_residues: np.ndarray,
        arr_agrc_frac_n_content_bg_residues: np.ndarray,
        arr_agrc_yield: np.ndarray,
        arr_soil_ef1_organic: np.ndarray,
        dict_arrs_agrc_frac_drywet: Dict['ModelVariable', np.ndarray],
        ind_rice: int,
        vec_soil_ef1_rice: np.ndarray,
        modvar_fert_mass: 'ModelVariable',
        modvar_ilu_mass: 'ModelVariable',
    ) -> Tuple[np.ndarray]:
        """Get EF1 components F_{CR} from AGRC

        Gets:
            F_{CR}, or residue input component
        
        Equations
        ---------
        F_{CR} from V4-C11-E11.6

        Notes
        -----
        (1) In Equation 11.6, total N from ABOVE-ground residues is sum of:
            1. residues left on fields 
            2. unburned residues, which is burned*(1 - C_f)

        (2) In Equation 11.6, total N from BELOW-ground residues is sum of:
            1. dry-matter fraction of total yields
            2. mass of above ground residues
        """

        # total AGRC residues
        arr_agrc_residues_total = (
            self.arrays_agrc.arr_agrc_residue_final_use_burned
            + self.arrays_agrc.arr_agrc_residue_final_use_energy
            + self.arrays_agrc.arr_agrc_residue_final_use_feed
            + self.arrays_agrc.arr_agrc_residue_final_use_field
        ...
        # In 11.6, total above-ground N residues is sum of
        #   1. residues left on fields 
        #   2. unburned residues, which is burned*(1 - C_f)
        arr_agrc_total_n_above_ground_residues = (
            ...
            + (
                self.arrays_agrc.arr_agrc_residue_final_use_burned
                * np.clip( # this shouldn't be strictly necessary, but just in case
                    1 - self.arrays_agrc.arr_agrc_combustion_factor,
                    0.0,
                    1.0,
                )
            )
        ...
        # multiply by N factor
        arr_agrc_total_n_above_ground_residues *= arr_agrc_frac_n_content_ag_residues


        # In 11.6, total below-ground N residues is sum of
        #   1. dry-matter fraction of total yields
        #   2. mass of above ground residues
        arr_agrc_total_n_below_ground_residues = (
            (
                arr_agrc_yield*self.arrays_agrc.arr_agrc_frac_dry_matter_in_crop
                + arr_agrc_residues_total
            )
            *self.arrays_agrc.arr_agrc_ratio_below_ground_biomass_to_above_ground_biomass
        )

        # multiply by N factor
        arr_agrc_total_n_below_ground_residues *= arr_agrc_frac_n_content_bg_residues


        # do above- and below-ground rice adjustments
        ...
        vec_agrc_total_n_below_ground_residues_dry = np.sum(
            arr_agrc_total_n_below_ground_residues
            *dict_arrs_agrc_frac_drywet.get(self.modvar_agrc_frac_dry, ), 
            axis = 1,
        )
        vec_agrc_total_n_below_ground_residues_wet = np.sum(
            arr_agrc_total_n_below_ground_residues
            *dict_arrs_agrc_frac_drywet.get(self.modvar_agrc_frac_wet, ), 
            axis = 1,
        )
        

        ##  GET TOTAL RESIDUE N BY TYPE AND CONVERT TO CORRECT UNITS
        ...
        ##  NOW WE CAN CALCULATE THE EF1 COMPONENT F_{CR}
    ...
    def get_soil_ef1_components_fon_fsn(self,
        arr_agrc_crop_area: np.ndarray,
        arr_land_use: np.ndarray,
        arr_lndu_frac_fertilized: np.ndarray,
        arr_soil_ef1_organic: np.ndarray,
        arr_soil_ef1_synthetic: np.ndarray,
        dict_arrs_agrc_frac_drywet: Dict['ModelVariable', np.ndarray],
        ...
        """Get EF1 components F_{ON} (organic N amendments) and F_{SN} 
            (synthetic N amendments) for AGRC and LNDU based on soil type for 
            each crop and land use type. Used to calculate:

        Equations
        ---------
        F_{ON} and F_{SN} from V4-C11-E11.1
        F_{ON} - V4-C11-E11.3
        """
        ...
        for modvar in self.modvar_list_agrc_frac_drywet:
            ...
            # get current factors
            arr_agrc_cur_wetdry_fertilized_crop = dict_arrs_agrc_frac_drywet.get(modvar, )*arr_agrc_crop_area
            ...
            # update dictionaries for use later
        ...
        for modvar in self.modvar_list_lndu_frac_drywet:
            ...
            vec_ef1_cur = arr_soil_ef1_organic[:, ind_soil]

            vec_soil_n2odirectn_fon += (vec_soil_frac_cur_drywet_pstr*vec_soil_n_fertilizer_use_organic)*vec_ef1_cur
            vec_soil_n2odirectn_fsn += (vec_soil_frac_cur_drywet_pstr*vec_soil_n_fertilizer_use_synthetic)*vec_ef1_cur
    ...
    def get_soil_fertilizer_vectors(self,
        ...
        vec_lsmm_nitrogen_to_fertilizer_urine: np.ndarray,
    ...
    def project(self,
        ...
        self.dict_arrs_agrc_frac_drywet = dict_arrs_agrc_frac_drywet
        ...
        # add in land use conversion and area
        (
            dfs_add, 
            df_agrc_frac_cropland, 
            df_land_use_ilu,
        ) = self.build_varsout_lndu_from_ilu(
            arr_agrc_frac_cropland,
            arr_land_use,
            arrs_lndu_land_conv,
        )
        ...
        # Additional variables from agriculture
        #   - Pos/Neg CO2 emissions from residues
        #   - Biomass increase/loss (woody biomass) emissions
        #   - CH4 emissions from decomposition 
        #   - CH4/N2O from residue burning
        #   - Residue quantities by pathway
        df_out += self.get_agrc_variables_additional(
            df_agrc_frac_cropland,
            df_land_use_ilu,
        )
        ...
        (
            ...
            vec_lsmm_nitrogen_to_fertilizer_urine,  # in terms of modvar_lvst_animal_mass
            vec_lsmm_nitrogen_to_pasture,           # in terms of modvar_lvst_animal_mass
        ) = self.get_lsmm_emissions(
            df_afolu_trajectories,
            arr_lvst_pop,
            dict_arrs_lndu_frac_drywet,
        )
