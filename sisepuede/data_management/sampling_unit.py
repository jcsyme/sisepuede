...
# prefixes in fields and varspecs
_PREFIX_FIELD_MAX = "max"
_PREFIX_FIELD_MIN = "min"
_PREFIX_FIELD_TRAJGROUP = "trajgroup"
_PREFIX_FIELD_TRAJMAX = "trajmax"
_PREFIX_FIELD_TRAJMIN = "trajmin"
_PREFIX_FIELD_TRAJMIX = "trajmix"

# varspecs from input templates
_VARSPEC_TYPE_LHS = "lhs"
_VARSPEC_TYPE_MAX = "max"
_VARSPEC_TYPE_MIN = "min"
_VARSPEC_TYPE_MIX = "mix"
...
class FutureTrajectories:
    ...
    def __init__(self,
        ...
        regex_id: re.Pattern = re.compile("(\D*)_id$"),
        regex_max: re.Pattern = re.compile(f"{_PREFIX_FIELD_MAX}_(\d*$)"),
        regex_min: re.Pattern = re.compile(f"{_PREFIX_FIELD_MIN}_(\d*$)"),
        regex_tp: re.Pattern = re.compile("(\d*$)"),
        regex_trajgroup: re.Pattern = re.compile(f"{_PREFIX_FIELD_TRAJGROUP}_(\d*)-(\D*$)"),
        regex_trajmax: re.Pattern = re.compile(f"{_PREFIX_FIELD_TRAJMAX}_(\D*$)"),
        regex_trajmin: re.Pattern = re.compile(f"{_PREFIX_FIELD_TRAJMIN}_(\D*$)"),
        regex_trajmix: re.Pattern = re.compile(f"{_PREFIX_FIELD_TRAJMIX}_(\D*$)"),
        # some internal vars
    ...
    def _initialize_input_database(self,
        ...
        for i, row in df_in.iterrows():
            ...
            if tg > 0:
                ...
                vs = regex_trajgroup.match(vs).groups()[1]
                new_tg_q = False
    ...
    def check_variable_trajectory_group(self,
        ...
        for varname, df in dfg:
            ...
            if no_baseline | multi_spec:
                ...
                msg = f"""Error in check_variable_trajectory_group: invalid specification 
        ...
        df_out = sf._concat_df(df_out, axis = 0)
    ...
    def clean_sampling_unit_input_df(self,
        ...
        if len(df_su_base) != n_req_baseline:
            ...
            msg = f"""Unable to initialize sample group{sg}: one or more variables and/or 
    ...
    def get_trajgroup_and_variable_specification(self,
        ...
        tg = None if not sf.isnumber(trajgroup_pass, skip_nan = True, ) else int(trajgroup_pass)
        var_spec = None
